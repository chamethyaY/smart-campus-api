...
### 10. Try to Delete Room with Sensors (409 error)
```bash
curl -X DELETE http://localhost:8082/api/v1/rooms/LIB-301
```
...
Returning **only IDs** reduces the response size and is faster to transmit, but forces the client to make additional requests to fetch details for each room individually. This increases client-side processing and the number of HTTP round trips.

In this implementation, full room objects are returned to provide a better developer experience and reduce the number of API calls needed.
...
Using `@QueryParam` for filtering (e.g., `GET /api/v1/sensors?type=CO2`) is considered superior to path-based filtering (e.g., `/api/v1/sensors/type/CO2`) for several reasons.

Query parameters are semantically meant for optional filtering, searching, and sorting of collections. They do not change the identity of the resource — `/api/v1/sensors` always refers to the sensors collection, and the `type` parameter simply narrows the results. Path parameters, on the other hand, are meant to identify a specific resource (e.g., `/api/v1/sensors/TEMP-001`).

Using path-based filtering creates ambiguity — is `CO2` a sensor ID or a type? Query parameters are also easier to combine (e.g., `?type=CO2&status=ACTIVE`) and are optional by nature, making the API more flexible and intuitive.
...
HTTP 404 Not Found means the requested URL/resource does not exist on the server. HTTP 422 Unprocessable Entity means the server understands the request and the URL is valid, but cannot process it because the content of the request is semantically incorrect.
